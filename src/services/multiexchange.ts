...
export class MultiExchangeAPI {
    ...
    constructor(exchange: ExchangeType, apiKey: string, apiSecret: string, testnet: boolean) {
        switch (exchange) {
            case 'binance':
    ...
    public async placeOrder(symbol: string, type: string, side: string, quantity: number, price: number) {
