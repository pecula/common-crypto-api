import { PositionResponse, AllOrders, ConvertedItem, ParseLoadmarketesLimits } from "../ExchangeInterface";
...
export function parseLoadmarketes(position: any): any | null {
  ...
  const symbolsArray: Item[] = position.symbols;
  const result = symbolsArray.reduce((acc: ConvertedItem, item: Item) => {
    let max: number;
    let min: number;
    let parseLoadmarketesLimits: ParseLoadmarketesLimits = { price: { min: 0, max: 0 }, amount: { min: 0, max: 0 }, cost: { min: 0, max: 0 }, market: { min: 0, max: 0 } };
